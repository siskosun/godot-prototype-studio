...
Align the player-facing brief and resolve visual reference intent first. Before scaffolding or substantive implementation, run [route and reuse](prototype-routing-and-reuse.md):

1. Compare Godot with a simple H5/browser-native route against the same acceptance contract. If H5 is materially simpler/faster and the stack is open, recommend it and ask once whether to switch. Respect an explicit stack lock and do not silently change technology.
2. After the stack is resolved, search for reusable source using the mechanic/state model, with special attention to current GPT-6/Astra indexes and same-stack projects. Verify original source, revision and license. Copy and baseline a close compatible same-stack project before adapting it. If an extremely similar reusable project exists in another stack, ask once whether to change stack.

Record the resolved route and reuse decision in the brief; use `templates/reuse_scan.md` when the search materially affects the implementation. Only then complete a buildable design. Use the optional Godot starter only when Godot remains selected and it saves more work than a reused baseline; it is instrumentation, not a finished game. If H5 is selected, route into the available H5/browser workflow and stop applying Godot-specific export/tool requirements.
...
Use gameplay-and-ux.md and art-direction.md to improve observed weaknesses. Continue into complete-session content, integrated art/audio, animation, UI, controls, and polish. Exercise real player input and inspect rendered presentation. Use game-qa-and-replay.md for proportionate risk/replay checks. Repair defects and retest the suites named by [change impact](change-impact.md); do not rerun an unrelated seed battery because a harness or document changed. Package and test the requested targets only. When Web is a chosen delivery, finish all declared post-export copies/patches before stamping a BUILD_ID, then serve it, run the applicable WEB_PREFLIGHT, and exercise the required browser path. Choose LOCAL_WEB_TEST or LAN_SHARE from the receiver context; LAN is not a quality prerequisite. For rooms/multiplayer/two-device promises, also use [multi-client verification](multiplayer-session-verification.md) and verify the joint session rather than inferring it from two independent clients. Keep source and Web hashes/evidence separate; when the working tree contains `export/web`, stage/package the editable source without that generated directory before source hashing. Use `package_and_report.py` as the freeze/hash/report entry so identities are not filled by hand. Public hosting is a separate action. Neither a foundation verdict nor an optional human review interrupts this commission.
