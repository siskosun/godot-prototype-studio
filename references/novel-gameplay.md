...
Use the **theme-removal test**: describe the loop without story, art, names, rewards, or content volume. If the claimed difference disappears, it is presentation novelty rather than new gameplay. Presentation novelty can still be valuable, but label it correctly.
...
When the design is unsettled, sketch a small set of causally distinct mechanic kernels. Usually three paper candidates are enough to expose repetition. Reject candidates that differ only in theme, quantity, reward values, VFX, or content. Implement only the few whose difference genuinely requires play to decide.

Prefer one familiar anchor plus one strong design delta. Combining several independent novelty axes in the first prototype makes failure hard to diagnose.
...
Run an adversarial pass that tries to collapse each candidate into a trivial policy. Do not let the proposing pass defend the idea with future content, progression, narrative, or polish. Revise or reject when the weakness is structural.
...
Retain the strongest verified baseline, rejected hypothesis and failure reason, and selected design delta. Once the mechanic is stable enough for the brief, freeze authoritative rules, add regression scenarios, then expand representative content and the live quality exemplar. Do not keep generating alternatives after decision-relevant uncertainty is resolved.
